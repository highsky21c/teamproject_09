...
def sign_up_view(request): #회원가입 화면이 실행될 때,
    if request.method == 'GET': #GET 방식으로
        ...
        if user: #유저가 있으면
            ...
            return render(request, '/user/sign-up.html') #회원가입 페이지만을 보여줌
        ...
        if password != password2: #패스워드 체크 : 같지 않다면
            return render(request, '/user/sign-up.html', {'error': '비밀번호 재확인'}) #회원가입 페이지
        else: #같다면
            if last_name == '' or username == '' or password == '' or email == '':
                return render(request, '/user/sign-up.html', {'error': '이름,아이디,비밀번호,이메일 필수'})
            ...
            if exist_username: # ID 중복이면
                return render(request, '/user/sign-up.html', {'error':'아이디 존재'}) #회원가입화면
            elif exist_email:  # email 중복이면
                return render(request, '/user/sign-up.html', {'error': '이메일 존재'}) #회원가입화면
            else: #중복이 아니면
                ...
                return redirect('/user/sign-in') #회원가입(사용자 저장) 완료후 로그인페이지로 이동
